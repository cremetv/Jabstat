  ...
  *************/
  logMembers: (server, member, status) => {
    ...
    targets.forEach(target => {
      ...
      db.execute(config, database => database.query(`INSERT INTO stat_members (userId, username, discriminator, nick, nicknames, avatar, avatarURL, displayColor, displayHexColor, game, status, roles, bot, deleted, createdAt, createdTimestamp, joinedAt, joinedTimestamp, agreedAt, introducedAt, updated)
                                                                        VALUES ('${target.user.id}', '${mysql_real_escape_string(target.user.username)}', '${target.user.discriminator}', '${mysql_real_escape_string(target.nickname)}', 'NICKNAMES', '${target.user.avatar}', '${target.user.avatarURL}', ${target.displayColor}, '${target.displayHexColor}', '${target.user.presence.game}', '${target.user.presence.status}', '${roles}', ${target.user.bot}, ${status}, '${target.user.createdAt}', '${target.user.createdTimestamp}', '${target.joinedAt}', '${target.joinedTimestamp}', 'AGREEDAT', 'INTRODUCEDAT', '${date.date}')
                                                    ON DUPLICATE KEY UPDATE username = '${mysql_real_escape_string(target.user.username)}', discriminator = '${target.user.discriminator}', nick = '${mysql_real_escape_string(target.nickname)}', nicknames = 'NICKNAMES', avatar = '${target.user.avatar}', avatarURL = '${target.user.avatarURL}', displayColor = ${target.displayColor}, displayHexColor = '${target.displayHexColor}', game = '${target.user.presence.game}', status = '${target.user.presence.status}', roles = '${roles}', bot = ${target.user.bot}, deleted = ${status}, createdAt = '${target.user.createdAt}', createdTimestamp = '${target.user.createdTimestamp}', joinedAt = '${target.joinedAt}', joinedTimestamp = '${target.joinedTimestamp}', agreedAt = 'AGREEDAT', introducedAt = 'INTRODUCEDAT', updated = '${date.date}'`)
      .then(rows => {
